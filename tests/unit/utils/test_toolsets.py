...
class TestGetEnabledToolsets:
    ...
    def test_all_keyword(self, monkeypatch):
        ...
        assert len(result) >= 21  # upstream baseline; fork may add more
    ...
    def test_all_toolsets_count(self):
        ...
        assert len(ALL_TOOLSETS) >= 21  # upstream baseline; fork may add more
    ...
    def test_all_toolsets_contains_jira_and_confluence(self):
        ...
        assert len(jira_toolsets) >= 15  # upstream baseline; fork may add more
        assert len(confluence_toolsets) >= 6  # upstream baseline
...
class TestToolsetTagCompleteness:
    ...
    def test_jira_tool_count(self, jira_tools):
        ...
        assert len(jira_tools) >= 49, f"Expected >= 49 Jira tools (upstream baseline), got {len(jira_tools)}"
